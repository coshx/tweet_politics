A First Dive into Natural Language Processing and Machine Learning
==================================================================


My last project as a summer intern at Coshx Labs Charlottesville involved a very exciting and very new topic for me: natural language processing. I've always been a reader, so I was happy to marry some of my academic interests together in one project. I've spent the last couple weeks with the odd feeling that school had started a bit too early: I've been reading textbooks, looking at Khan Academy and OpenCourseWare videos, and in general feeling very inadequate about my math skills as I learn needed background material in probability, information theory, and linguistics. I love learning new things though, so I really can't complain.

To test what I've been learning, my boss, Ben, gave me a fun little side project: build a small webapp that automatically classifies tweets as political or apolitical. While at first glance this was a fairly simple project, there's actually a lot going on. The app had to:
...
* **Classify the tweets using the features extracted.** As an avid Python programmer, I have heard of the [Natural Language Toolkit][NLTK] library, and I was excited that I would finally have the chance to use it. Using NLTK meant that I didn't actually have to write a classifier myself; I just had to feed a training corpus (i.e., a set of tweets that are already classified) to one of NLTK's built-in classifiers and let it do the hard math for me. The downside to this approach (usually called *supervised machine learning*) was that I had to manually classify tweets to build the corpus -- and a lot of tweets at that, if I were to build a robust corpus that captures the breadth of language used by Twitter users.

* **Display the results.** All the webapp has to do is display a tweet and its classification, so this can be done in one page. My natural impulse was to use [Django][], but using an MVC framework seemed overkill to display a single page; instead, I opted to use the [Flask][] microframework.
...
Note that I set the number of tweets to fetch using the `count` argument; 200 is the maximum number of tweets you can fetch in one method call. The training corpus should have more than 200 tweets, so keep calling `GetUserTimeline` and append the `tweets` list:


    >> tweets += api.GetUserTimeline(screen_name="steveklabnik", count=200, max_id=tweets[-1].id)
...
First, let's convert the tweet objects into small dictionaries, since tweet objects have a lot of extra data that we don't really need:
    ...
    >> import json
    >> json_tweets = [{"id": tweet.id, "text": tweet.text, "political": False} for tweet in tweets]
...
Note that I added a `political` key in the tweet dictionaries, which is the classification for that tweet. Since you have to manually build the corpus, you have to change this manually for every tweet. Yes it sounds painful to classify ~3,000 tweets, I know. To make the work easier on me, I wrote a script that loads the JSON file of serialized tweets, displays the tweets one by one and prompts me to classify the tweet as political/apolitical, and then serializes the updated tweet dictionaries back to the JSON file. I've posted it in a [gist][classify-manual] for your perusal.
...
From what I've experienced in this project, feature extraction is largely an idiosyncratic process -- that is, the kind of features that you need depend on the project domain. In this case, I hope that people working with tweets -- in general, not just classifying whether tweets are political/apolitical -- would find the following section informative. Note, however, what I did here came from hands-on experiments; for the most part, there's not much math involved here. Some might be annoyed at the lack of rigor, but note that this was a small and fairly trivial project with a short timeframe for completion.

For this project, I focused on extracting keywords from tweets to build a feature set. These keywords could then be used by the classifier to determine whether a tweet is political or apolitical. For example, a tweet that contains the word "government" or "senate" is probably political. The basic strategy is this: rank the keywords, and then create a correlation between a keyword and a classification. Ranking the keywords is the main goal of feature extraction creating a correlation between keywords and classifications is the main goal of classifying the tweets. We will focus on ranking keywords for this section.
...
1. Clean up the raw text of a tweet
2. Tokenize the cleaned text
3. Rank keywords in a tweet, for all the tweets in the corpus


### Cleaning up raw text

Tweets are notoriously unstructured -- you don't approach writing a tweet as seriously as you do writing an essay -- and that can pose a challenge for building informative feature sets. A tweet with the word "goverment" might not be classified as political, but we can readily tell that if it was spelled correctly -- "government" -- there would be a higher probability that it would be classified as political. Tweets can also have a lot of extraneous text that probably would not be useful for determining whether they are political. Take this one, for example:


    RT @glynmoody: Snowden\'s father writes open letter to NSA whistleblower in Moscow - http://t.co/jyRnRnmajo is #snowden or #clapper the "gre\u2026


It probably doesn't matter that this is a retweet from @glynmoody. Words like "to" and "in" contain little semantic content and serve nothing more as scaffolding. Given this, the two main goals of cleaning up raw text are to 1) normalize the text and 2) remove extraneous text.
...
1. **Convert Unicode to ASCII.** This is probably a bad idea for less trivial projects -- don't do this if you're working in a language with a non-latin script of course, like Chinese -- but just for this project we'll just convert tweets into ASCII strings and ignore non-ASCII characters to make things simple. For example, take this tweet:
...
6. **Convert hashtags.** Hashtags are usually important keywords, so it's important we normalize them to be usable. A common pattern of hashtags is where a phrase is written in camel case; that is, the separate words of the hashtag are capitalized, but there are no spaces in between. It would be useful then to separate the words, so that each maybe be used as a possible keyword. There is also the common pattern where the words aren't capitalized (e.g., #winninglife versus #WinningLife or #winningLife), but for now we'll ignore that since it's much harder to separate words for that type of hashtag. Now the tweet looks like:
...
Tokenization basically means to split the text into a list of tokens -- which, in this case, are analogous to words. Python's NLTK library makes this process trivial, as it already has several built-in tokenizers that we can use. For this project, I used the whitespace tokenizer, which, as its name implies, splits the text according to whitespace. The other tokenizers mostly differ in how they tokenize punctuation, but since we've already removed all the punctuation from the text, we can just split the text by spaces. Tokenization code looks something like this:
    ...
    >> print cleaned_text
    >> ["hope", "you", "had", "a", "blast", "here", ""is", "a", "link", "to", "sheryl", "pics", "dont", "forget", "my", "email", "is", "team", "carribean"]
...
1. **Expand contractions.** This is another normalization step, and a fairly self-explanatory one at that: we take a contraction token and split it into its constituent tokens. Now the list of tokens looks like:
        ...
        ["hope", "you", "had", "a", "blast", "here", ""is", "a", "link", "to", "sheryl", "pics", "do", "not", "forget", "my", "email", "is", "team", "carribean"]
...
2. **Remove stopwords.** Remember those words like "to" and "in" that don't have much semantic content? They are actually called "stopwords," and, as you've probably guessed, we're going to remove them ([here][stopwords] is a pretty exhaustive, but by no means canonical, list of common English stopwords). Now the tweet looks like this:
        ...
        ["hope", "blast", "link", "sheryl", "pics", "forget", "email", "team", "carribean"]


    This is a pretty good, informative list of keywords. We could stop here, but I added one more step.
...
What does the application of tf-idf actually look like? Let's see an example, using the list of tokens from the previous example:
...
As I have previously mentioned, we don't need to actually code a classifier; instead, we'll use one of NLTK's built-in classifiers and feed it a training corpus. Using this training corpus, the classifier automatically correlates certain keywords with a certain classification. NLTK has three types of classifiers built-in; for this project, I used the naive Bayes classifier.

First off, let's create this training corpus. This corpus consists of *tagged feature sets*, or feature sets coupled with a classification. This is where the hard work of classifying all those tweets come in. To build this corpus, we can use the feature set builder we used previously:


    >> tagged_features = tf.build_tagged_featureset(tweets)


Note that `build_tagged_featureset` looks for a "political" key in the `tweets` dictionary; that is, you must already have previously classified the tweet as political/apolitical.
...
With a training corpus at hand, let's create a classifier:
    ...
    >> classifier = nltk.NaiveBayesClassifier.train(tagged_features)
...
Voila! It's that easy using NLTK, though granted building a feature set took a while. You can now use the classifier to automatically classify tweets. Let's see it in action:
    ...
    >> tweet = {"text": u"RT @bob: Hope you had a blast! Here is a link to Sheryl's pics: http://t.co/skj2o Don't forget my email bob@gmail.com #TeamCarribean ☺"}
    >> features = tf.build_featureset([tweet])[0]
    >> print classifier.classify(features)
    >> False


If you're curious about what keywords the classifier looks for to tag tweets as political, here's a list generated by a handy classifier method:
...
Given that, at the time of this writing, the NSA PRISM domestic surveillance scandal is still broiling, it is no surprise that "nsa" and "prism" are the top keywords to identify a political tweet. Of course, the problem is that once this scandal wanes from the public conversation -- and therefore from Mr. Klabnik's Twitter feed -- the classifier would start to get less and less accurate. One way to alleviate this is to expand the corpus to include older tweets and so that the classifier can pick up important keywords that are agnostic to current events and yet still good signifiers for a political tweet.

So that's pretty much it. The only topic I didn't discuss is writing a simple webapp to display the results of the classifier, but that's a bit off topic -- and I'm sure you, dear reader, already have your preferred web framework to use and would rather not be bored with a primer to web development.
...
If you are curious about what the webapp I wrote looks like, you can find it here: [LINK TO steveklabnik-tweets-politics.herokuapp.com]
...
* Michael Collins's Coursera class on NLP. Granted, I only took a week's worth of material from this class, so I cannot say much about it, but from the material it covered, I can tell you that this class is very math intensive. It's best that you learn a bit of probability and some information theory first before taking this class; this is a math-intensive class.

* Foundations of Statistical Natural Language Processing by

* Khan Academy videos on Information Theory
